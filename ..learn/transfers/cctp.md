...
You can use Wormhole's CCTP-powered USDC bridging by embedding the [Connect Widget](/docs/products/connect/overview/){target=\_blank} or by integrating the [TypeScript SDK](/docs/tools/typescript-sdk/get-started/){target=\_blank} directly.
...
This attestation delivery may be difficult or impossible in some contexts. For example, in a browser context, the user doesn't wish to wait for finality to deliver the attestation. To address this difficulty, the Wormhole CCTP relayer may be used either with the [Wormhole Connect Widget](/docs/products/connect/overview/){target=\_blank} or more directly with the [Wormhole TypeScript SDK](/docs/tools/typescript-sdk/get-started/){target=\_blank}.
