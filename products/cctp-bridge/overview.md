...
| Aspect | CCTP (native USDC) | WTT (wrapped tokens) |
|---|---|---|
...
| Payload | Optional payload; executed on the destination when USDC is minted | Transfer with payload; executed by the recipient contract during redemption |
...
- Use CCTP when sending USDC between [CCTP-enabled chains](/docs/products/cctp-bridge/reference/supported-networks/){target=\_blank}. This route appears only if both chains support Circle CCTP and the asset is native USDC.
- Use CCTP with a payload when you want the destination contract to run logic as the USDC is minted on the target chain. 
- If you only need to send data without moving USDC, use [Messaging](/docs/products/messaging/overview/){target=\_blank}.
- If the destination isn't CCTP-enabled for USDC, the transfer routes via [WTT](/docs/products/token-bridge/overview/){target=\_blank}. You can still include a payload that the recipient contract executes during redemption.
