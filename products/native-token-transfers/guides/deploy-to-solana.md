...
- **Set to token authority PDA:**
    ```bash
    ntt solana set-mint-authority --token INSERT_TOKEN_ADDRESS --manager INSERT_YOUR_NTT_PROGRAM_KEY_PAIR
    ```

- **Set to SPL Multisig:**
1. Create valid SPL Multisig:
```bash
ntt solana create-spl-multisig INSERT_MINTER_PUBKEY_1 .. INSERT_MINTER_PUBKEYN
```

2. Set to created SPL Multisig:
```bash
ntt solana set-mint-authority --token INSERT_TOKEN_ADDRESS --manager INSERT_YOUR_NTT_PROGRAM_KEY_PAIR --multisig INSERT_MULTISIG_ADDRESS --yes --payer INSERT_YOUR_KEYPAIR_JSON
```
