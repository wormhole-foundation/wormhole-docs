...
To enable NTT in your app, follow these steps:
 ...
 2. Add metadata for each token in [`tokensConfig`](#custom-tokens). Every token listed in `nttRoutes` must have a corresponding entry in `tokensConfig`, either as a built-in or a custom token.
