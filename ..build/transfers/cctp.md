...
??? code "Circle Integration contract"
    ```solidity
    --8<-- 'code/build/transfers/cctp/CircleIntegration.sol'
    ```
...
??? code "Token Messenger contract"
    ```solidity
    --8<-- 'code/build/transfers/cctp/TokenMessenger.sol'
    ```
...
- **`depositForBurn`** - deposits and burns tokens from the sender to be minted on the destination domain. Minted tokens will be transferred to `mintRecipient`
    ...
    ??? interface "Emits"
        ...
        --8<-- 'text/build/transfers/cctp/DepositForBurn-event.md'
...
??? code "Message Transmitter contract"
    ```solidity
    --8<-- 'code/build/transfers/cctp/MessageTransmitter.sol'
    ```
...
- **`sendMessage`** — sends a message to the destination domain and recipient. It increments the `nonce`, assigns a unique `nonce` to the message, and emits a `MessageSent` event
    ...
    ??? interface "Emits"
        ...
        --8<-- 'text/build/transfers/cctp/MessageSent-event.md'
...
??? code "Token Minter contract"
    ```solidity
    --8<-- 'code/build/transfers/cctp/TokenMinter.sol'
    ```
...
??? code "CCTP Sender contract"
    ...
    ```solidity
    --8<-- 'code/build/transfers/cctp/CCTPSender.sol'
    ```
...
```solidity
--8<-- 'code/build/transfers/cctp/sendUSDCWithPayloadToEvm.sol'
```
...
```solidity
--8<-- 'code/build/transfers/cctp/sendCrossChainDeposit.sol'
```
...
??? code "CCTP Receiver contract"
    ...
    ```solidity
    --8<-- 'code/build/transfers/cctp/CCTPReceiver.sol'
    ```
...
```solidity
--8<-- 'code/build/transfers/cctp/receivePayloadAndUSDC.sol'
```
