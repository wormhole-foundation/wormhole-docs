...
The standard relayer favors simplicity over flexibility. All computation must happen on-chain, as the relayer performs no off-chain logic. As a result, advanced workflows - like complex conditional logic, multi-step workflows, or gas-intensive computations - cannot be offloaded. Additionally, the Wormhole relayer network currently supports only EVM-compatible blockchains. Non-EVM chains (Solana, Sui, etc.) aren’t covered by the standard relayer.
