...
When integrating Wormhole messaging, developers face a choice between manual (client-side) relaying and automated relaying. These refer to who/what performs the delivery of the VAA to the target chain.

- **Manual Relaying (Client-Side)**: This approach puts the burden on the user or their client (e.g., a dApp or wallet) to carry out all cross-chain steps. After an action on Chain A produces a VAA, the user must manually fetch that VAA (typically via a Wormhole API or explorer) and then submit it in a transaction on Chain B. No specialized backend is needed. The “relayer” is essentially the user themselves and their web browser or wallet. The upside is simplicity in architecture (no extra services to run) and no additional fees beyond the target chain’s transaction fees. However, the user experience is poor for anything beyond basic demos. Users have to sign multiple transactions and maintain funds on each chain involved. This can be cumbersome and error-prone, as users may not understand why they need to perform an extra step or could drop off during the process. In summary, manual relaying is fine for testing and MVPs, but it’s usually not ideal for polished products.
- **Automated Relaying**: n this approach, the cross-chain delivery is handled automatically by some service or network, not the end-user. From the user’s perspective, the message “just arrives” on the target chain without manual intervention. Automated relaying significantly improves the user experience — for example, a user can initiate an asset transfer with one click and then wait for the funds to arrive on the destination chain. There are two ways to achieve automated relaying:
    - **Build a relayer service (custom backend)**: i.e., run an off-chain service that listens for VAAs and forwards them. This gives complete control (gas optimization, batch transactions, retry handling, etc.), but requires development and maintenance of a backend.
    - **Use a relayer network provided by Wormhole**: i.e., tap into an existing decentralized relayer service that Wormhole offers, which requires minimal effort to integrate and no infrastructure to run. Wormhole’s relayer solutions enable users to request the delivery of messages through on-chain calls, with an untrusted external delivery provider handling the rest. This saves integrators from running a service at the cost of some fees. In either case, the goal is the same: the user is abstracted away from the cross-chain complexity, resulting in a smoother experience
