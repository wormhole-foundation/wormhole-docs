...
2. **Guardian Network**: [Guardians](/docs/protocol/infrastructure/guardians/){target=\_blank} validate these messages and sign them to produce [Verifiable Action Approvals (VAAs)](/docs/protocol/infrastructure/vaas/){target=\_blank}. For chains configured for delegated observation, a delegated subset performs direct observation while Canonical Guardians wait for delegated quorum before signing. The final VAA remains a standard 13-of-19 multisignature attestation.
...
- **Guardian Network**: Validators that exist in their own P2P network. Guardians observe and validate messages emitted by the Core Contract to produce VAAs. For full Guardian Set chains, all Guardians perform direct on-chain observation. For chains configured for delegated observation, a delegated subset performs observation and broadcasts signed observations to the rest of the network, after which Canonical Guardians wait for delegated quorum before signing.
