...
- Testnet tokens for two supported chains. This example uses [testnet AVAX for Avalanche Fuji](https://core.app/tools/testnet-faucet/?subnet=c&token=c){target=\_blank} and [testnet ETH for Base Sepolia](https://docs.base.org/docs/tools/network-faucets/){target=\_blank} and can be adapted to any supported chains.
...
4. Follow the prompts in the terminal. This example uses Avalanche Fuji as the source chain, Base Sepolia as the target, [Avalanche Fuji testnet USDC](https://developers.circle.com/stablecoins/usdc-contract-addresses#testnet){target=\_blank}, and a developer wallet as the recipient address. You will see terminal output similar to the following:
