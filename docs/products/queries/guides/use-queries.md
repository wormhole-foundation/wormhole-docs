...
Here's an example for Ethereum Mainnet, where the `-a` parameter is the [Core Contract address](/docs/reference/contract-addresses/#core-contracts){target=\_blank} on that chain.
