...
The documentation source files are written in [Markdown](https://daringfireball.net/projects/markdown) and generally follow the [Google developer documentation style guide](https://developers.google.com/style). This guide covers how to contribute to the documentation, including serving a local version of the site, adding new pages and directories, managing images and code snippets, and a few SEO tips.
...
To create a new section of content, you will first need to create a subdirectory with the desired name of the section. The root directory, and every subdirectory, must contain the following files:
...
- The `template` defines the template to be used. These optional fields automatically generate cards based on the title and description of each page or subsection within the section. The `main` template uses different styling and allows you to provide a disclaimer for all of the content within that section. This is for root-level sections only.

The `subsection` template is for all other sections that are not root-level.
...
Snippets are used to manage reusable lines of code or text. They are organized to mirror the structure of the docs site and stored under the root-level `.snippets` directory. For example, to add a code snippet to the page `build/build-apps/wormhole-sdk.md`, you would place the code snippet in the folder `.snippets/code/build/build-apps/wormhole-sdk`
