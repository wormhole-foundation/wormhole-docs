...
The documentation source files are written in [Markdown](https://daringfireball.net/projects/markdown){target=\_blank} and generally follow the [Google developer documentation style guide](https://developers.google.com/style){target=\_blank}. This guide covers how to make a contribution to the documentation including serving a local version of the site, adding new pages and directories, managing images and code snippets, and a few SEO tips.
...
You will likely want to spin up a local version of the documentation site to preview your changes. This guide will cover the steps needed to serve a local version: 
...
This command will install all of the depencies included in the `requirements.txt` file.
...
Assuming this was added under the 'Build' directory, this example would result in adding 'Build Apps' as a subdirectory to 'Build' with two files inside, `index.md` and `wormhole-sdk.md`, and two additional subdirectories named `queries` and `connect`. 
...
2. Modify content as desired. Remember to place images and code snippets in the appropriate folders (see the next sections for details)
3. Once you commit and pushed all of your changes, open a pull request for the new content branch against the `main` branch
4. Monitor notifications and/or the pull request for feedback from code owners. At least one approval is required before content is merged
