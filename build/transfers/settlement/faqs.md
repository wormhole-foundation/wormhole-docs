...
If the slippage tolerance is set too low, the user may receive USDC on the destination chain instead of the intended swap outcome. However, the 4 basis points (bps) fee is non-refundable, as the service provided by LL solvers (ensuring front-finality) is separate from the composing protocol’s services, such as swaps or deposits.
