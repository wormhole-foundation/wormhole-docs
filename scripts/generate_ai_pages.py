...
def fetch_local_snippet(snippet_ref, snippet_directory, _variables):
...
def replace_snippet_placeholders(markdown, snippet_directory, yaml_data):
    ...
    def fetch_snippet(snippet_ref: str) -> str:
        resolved = resolve_markdown_placeholders(snippet_ref, yaml_data)
        if resolved.startswith("http"):
            return fetch_remote_snippet(resolved)
        return fetch_local_snippet(resolved, snippet_directory, yaml_data)
    ...
    def replace_line_match(match: re.Match) -> str:
        ...
        if not indent:
            return snippet_content
        indented = [f"{indent}{line}" if line else "" for line in snippet_content.split("\n")]
        return "\n".join(indented)
...
def wrap_yaml_code_fences(markdown: str) -> str:
    """Wrap ```yaml fences in Jinja raw blocks to keep templating engines neutral."""
    ...
    while i < len(lines):
        ...
        if fence_match and fence_match.group(2) and fence_match.group(2).lower() in ("yaml", "yml"):
            indent = fence_match.group(1)

            closing_idx = i + 1
            while closing_idx < len(lines):
                if re.match(rf"^{re.escape(indent)}```\s*$", lines[closing_idx]):
                    break
                closing_idx += 1
            ...
            j = len(wrapped_lines) - 1
            while j >= 0 and wrapped_lines[j].strip() == "":
                j -= 1
            already_wrapped = j >= 0 and wrapped_lines[j] == f"{indent}{{% raw %}}"

            if not already_wrapped:
                wrapped_lines.append(f"{indent}{{% raw %}}")

            for block_idx in range(i, closing_idx + 1):
                wrapped_lines.append(lines[block_idx])

            if not already_wrapped:
                wrapped_lines.append(f"{indent}{{% endraw %}}")
...
def main():
    ...
    for file_path in files:
        ...
        if DRY_RUN:
            ...
            # quick counts
            snip_ct = len(SNIPPET_TOKEN_REGEX.findall(body))
